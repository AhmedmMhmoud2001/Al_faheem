...
export default function QuestionEdit() {
  ...
  async function submit(e) {
    ...
    setBusy(true);
    try {
      await api.patch(`/admin/questions/${id}`, buildPayload(form));
      navigate('/questions');
    } catch (err) {
  ...
  if (!form) return <p className="p-6 font-bold text-red-600">{error || t('questions.notFound')}</p>;

  const src = mediaSrc(form.imageUrl);

  return (
    <FormPage title={t('questions.editTitle')} backTo="/questions" maxWidth="max-w-4xl">
          ...
          <div className="flex flex-wrap gap-4">
            {[0, 1, 2, 3].map((i) => (
              <label key={i} className="flex items-center gap-2 text-sm font-bold text-[var(--app-fg)]">
                <input type="radio" name="correct-edit" checked={Number(form.correctIndex) === i} onChange={() => setForm({ ...form, correctIndex: i })} />
